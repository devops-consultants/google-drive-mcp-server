...
Each entry SHALL include: `name`, `path`, `type` ("file" or "folder"), `size` (bytes, null for folders), and `modified` (ISO 8601 timestamp). ETags are not included in list results for performance; use `file_info` to retrieve ETags for specific files.
